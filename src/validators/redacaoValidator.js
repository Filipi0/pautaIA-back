class RedacaoValidator {
  ...
  const linhasSanitizadas = linhasFront.map(linha => this.sanitizarLinha(linha));
  const linhasComTexto = linhasSanitizadas.filter(linha => linha.trim() !== "");
  const totalLinhasEscritas = linhasComTexto.length;

  if (totalLinhasEscritas > 0 && totalLinhasEscritas < 7) {
    avisos.push(`Aviso: A redação tem apenas ${totalLinhasEscritas} linhas escritas. O ENEM anula redações com menos de 7 linhas.`);
  }

  if (linhasSanitizadas.length > 30) {
    avisos.push(`Aviso: O texto excedeu 30 linhas (total enviado: ${linhasSanitizadas.length}). O ENEM avalia apenas até a linha 30.`);
  }
  ...
  if (linhasComErro.length > 0) {
    ...
    .join('; ');
    erros.push(`Erro: ${linhasComErro.length} linha(s) ultrapassaram o limite visual de caracteres. ${detalhes}`);
  }

  const válido = erros.length === 0;
  return { válido, erros, avisos, totalLinhasEscritas, linhasComErro, linhasSanitizadas };
  }
  ...
  for (const secao of secoes) {
    ...
    linhas.forEach((numeroLinha) => {
      ...
      mensagens.push(`Linha ${numeroLinha} em '${secao}' está fora do intervalo do rascunho`);
      }
